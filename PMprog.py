...
class PM(object):
    ...
    def try_login(self, event = None):
        ...
        if self.intro_password.get() == "main":
            self.main_menu()            
        else:
...
You can always change mode by pressing the bottom right button in the main menu.
    ...
    def main_menu(self):
        ...
        upload_button = self.upload_button = tk.Button(root, text = "Upload", width = 11, height = 2, command = self.upload)
...
glufs - GLuFS """)
    ...
    def add_event(self, event = None):
        ...
        new_score = {"time":datetime.now().replace(microsecond=0),
                   "quest_number":0,
                   "quest_value":0,
                   "bonus":0,
                   "pitty_points":0,
    ...
    def make_plot(self, event = None):#Makes the plot
        ...
        time_list_fake=[]
        day0 = datetime.strptime(time_list[0], "%Y-%m-%d %H:%M:%S")
        

        for entry in time_list:
            
            current_time = datetime.strptime(entry, "%Y-%m-%d %H:%M:%S")

            hours_since_day0 = (current_time - day0).total_seconds() / 3600  # Difference in hours
            time_list_fake.append(hours_since_day0) 
            ...
            #time_list_fake.append((int(entry.split(" ")[0].split("-")[1])*31+int(entry.split(" ")[0].split("-")[2]))*24+int(entry.split(" ")[1].split(":")[0])-day0*24)
                #Or days
